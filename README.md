...
交易策略差异：
- 止盈：阶梯式回撤止盈（涨15%触发，15%~30%固定回撤15%，≥30%中点止盈法），与潜伏型一致
- 止损：-30%（毕业币波动相对小，止损更紧）
...
| # | 策略 | 条件 | 说明 |
|---|------|------|------|
| 1 | 阶梯式回撤止盈 | 涨 15% 触发跟踪; 15%~30% 从最高价回撤 15% 卖出; ≥30% 中点止盈法 | 保住本金, 又不被好币洗出来 |
...
止盈分两个阶段，兼顾保本和不被好币洗出来：

| 阶段 | 触发条件 | 止盈方式 | 说明 |
|------|----------|----------|------|
| 阶段1 | 最高盈利 15%~30% | 从最高价回撤 15% 即卖出 | 固定回撤, 保住本金 |
| 阶段2 | 最高盈利 ≥30% | 中点止盈法: 跌到 (买入价+最高价)/2 卖出 | 让利润奔跑, 回撤到中点落袋 |
...
| `tp_midpoint_pct` | 30 | 中点止盈法启用阈值 (%, 最高盈利达到此值后从固定回撤切换到中点止盈) |
