...
扫描 [four.meme](https://four.meme) 和 [flap](https://flap.sh/bnb) 平台上新发行的 BSC 代币，链上发现 + 队列淘汰制 + 标签制精筛 + 精筛后防线，推送到钉钉，可选自动交易。
...
3. **淘汰检查** (~数秒): DexScreener 批量查价(含交易量/买卖笔数/涨跌幅/Boost) + GeckoTerminal 持币数 + Detail API → 永久淘汰弃盘币
3b. **K线修正**: 对持币≥50 的存活代币拉 GT 15min K线 → 修正 peakPrice + 记录 klineHigh/klineLow (过山车检测)
4. **精筛** (瞬时): 标签制精筛 — 统一通道, 基础标签全部满足(持币≥10/进度≥8%或流动性≥$5k/仿盘≥3/未崩盘/社交≥1, flap豁免: 持币≥20+进度≥15%时跳过社交) + 单动能触发(最近3轮内持币增长或价格上涨, 任一即可)
...
| flap | `0xe2cE6ab0...` (Portal) | `8888` (标准) / `7777` (税币) | ❌ 无 Detail API (进度通过链上 `getToken()` 读取) |

### flap 平台交易架构

flap 平台的 Portal 合约对外部直接调用 `buy()`/`sell()` 有 access control（返回 `0xac5f6092`），所有代币（标准币 8888 和税币 7777）都必须通过 Router 合约交易：

| 合约 | 地址 | 用途 |
|------|------|------|
| Portal | `0xe2cE6ab80874Fa9Fa2aAE65D277Dd6B8e65C9De0` | 状态查询 (`getTokenV5`)、事件发射 (`TokenBought`/`TokenSold`) |
| Router | `0x296B00198Dc7eC3410e12da814D9267bB8dF506A` | 实际交易入口 (买入/卖出) |

Router 合约细节：
- Proxy 合约，implementation 未 verify，无公开 ABI
- 统一 swap 函数 selector: `0x27772d13`
- 买入: tokenIn=NATIVE(BNB), tokenOut=代币, msg.value=BNB金额
- 卖出: tokenIn=代币, tokenOut=NATIVE(BNB), msg.value=0, 需先 approve Router
- 路由参数中包含 WBNB 和 Portal 地址
- 买入 gas: ~330k (设置 500k)，卖出 gas: ~450-570k (设置 700k)
- 不管代币的 quote token 是什么（BNB/USD1/USDT），Router 统一用 BNB 支付/收取
...
| 社交 | ≥ 1 | 有基本运营 |
| 单动能 | 最近3轮内持币数有增长 或 价格有上涨（任一即可） | 确认增长趋势，过滤静态达标的僵尸币。首轮豁免：首轮入队无历史数据时，持币≥20 + 进度≥15%/流动性≥$5k 视为有动能 |
...
| # | 策略 | 条件 | 说明 |
|---|------|------|------|
| 1 | 阶梯式回撤止盈 | 涨 15% 触发跟踪; 15%~30% 从最高价回撤 15% 卖出; 30%~300% 三分点止盈法; ≥300% 中点止盈法 | 保住本金, 给好币更多空间, 大涨锁利润 |
| 2 | 动能衰竭止盈 | ≥2 个动能指标恶化 且当前盈利 | 趁还有利润赶紧跑 |
| 3a | 超期清仓 (亏损) | 持仓 ≥ 48h 且仍亏损 | 给了足够时间还亏就别等了 |
| 3b | 超期清仓 (低效) | 持仓 ≥ 72h 且盈利 < 500% (5倍) | 资金效率太低，回收去抓下一个 |
| 4 | 兜底止损 | 亏损 ≥ 30% | 保住本金, 不给太多空间 |
| 5 | 重买冷却 | 盈利平仓后 12h, 亏损平仓后 24h | 避免反复追同一个币 |

### 阶梯式回撤止盈

止盈分三个阶段，兼顾保本、给好币空间和锁住大涨利润：

| 阶段 | 触发条件 | 止盈方式 | 说明 |
|------|----------|----------|------|
| 阶段1 | 最高盈利 15%~30% | 从最高价回撤 15% 即卖出 | 固定回撤, 保住本金 |
| 阶段2 | 最高盈利 30%~300% | 三分点止盈法: 跌到 (买入价+最高价)/3 卖出 | 给好币更多空间, 不被洗出来 |
| 阶段3 | 最高盈利 ≥300% | 中点止盈法: 跌到 (买入价+最高价)/2 卖出 | 大涨后锁住大部分利润 |

### 动能衰竭止盈

基于历史数据分析发现：持币数、流动性、进度三个指标同时恶化时，代币后续几乎必跌。0 个指标恶化的精筛代币正收益率远高于 2 个指标恶化的。

监控线程每轮采集持币数（fm_detail）、流动性（DexScreener）、进度（fm_detail），记录到 momentum 表。当以下指标中 ≥2 个同时触发，且当前持仓盈利 > 0 时，立即止盈：

| 动能指标 | 恶化阈值 | 适用范围 | 计算方式 |
|----------|----------|----------|----------|
| 持币数 | 从峰值跌 >30% | 所有代币 | 相对值: (峰值-当前)/峰值 |
| 流动性 | 从峰值跌 >30% | 仅已毕业 (流动性>$100) | 相对值: (峰值-当前)/峰值 |
| 进度 | 从峰值跌 >15 个百分点 | 仅未毕业 (峰值进度>5%) | 绝对值: 峰值进度-当前进度 |

| 配置参数 | 默认值 | 说明 |
|----------|--------|------|
| `tp_trigger_pct` | 15 | 回撤止盈触发阈值 (%, 涨幅达到此值后激活止盈跟踪) |
| `tp_midpoint_pct` | 30 | 三分点止盈法启用阈值 (%, 最高盈利达到此值后从固定回撤切换到三分点止盈) |
| `tp_full_midpoint_pct` | 300 | 中点止盈法启用阈值 (%, 最高盈利达到此值后从三分点切换到中点止盈) |
| `tp_drawdown_pct` | 15 | 固定回撤止盈幅度 (%, 15%~30%区间从最高价回撤此比例即卖出) |
| `momentum_tp_enabled` | true | 是否启用动能衰竭止盈 |
| `momentum_min_bad_signals` | 2 | 触发止盈的最少恶化指标数 |
| `expire_loss_hours` | 48 | 亏损超期时间 (小时) |
| `expire_underperform_hours` | 72 | 低效超期时间 (小时) |
| `expire_min_profit_pct` | 500 | 超期最低盈利要求 (%) |
| `stop_loss_pct` | -30 | 默认止损 (%, 动能健康时使用) |
| `momentum_stop_loss_pct` | -25 | 动能恶化止损 (%, 动能全部恶化时提前止损) |
| `rebuy_cooldown_profit_hours` | 12 | 盈利平仓后重买冷却 (小时) |
| `rebuy_cooldown_loss_hours` | 24 | 亏损平仓后重买冷却 (小时) |
...
| **基础标签** | | |
| `TAG_BASE_MIN_HOLDERS` | 10 | 基础: 持币数 ≥ 10 |
| `TAG_BASE_MIN_PROGRESS` | 0.08 | 基础: 进度 ≥ 8%（仅未毕业币） |
| `TAG_BASE_MIN_LIQUIDITY` | 5000 | 基础: 流动性 ≥ $5k（仅已毕业币） |
| `TAG_BASE_MIN_COPYCAT` | 3 | 基础: 仿盘数 ≥ 3 |
| `TAG_BASE_MAX_CRASH_PCT` | 0.35 | 基础: 近三期最高点跌幅 < 35% |
| `TAG_BASE_MIN_SOCIAL` | 1 | 基础: 社交 ≥ 1（flap 豁免: 持币≥20 且 进度≥15%/已毕业） |
| `TAG_FLAP_SOCIAL_EXEMPT_HOLDERS` | 20 | flap 社交豁免: 持币数 ≥ 20 |
| `TAG_FLAP_SOCIAL_EXEMPT_PROGRESS` | 0.15 | flap 社交豁免: 进度 ≥ 15%（未毕业） |
| `FLAP_SOCIAL_PENDING_MAX_ROUNDS` | 2 | flap 社交 pending 最大缓冲轮数（超时淘汰） |
| `TAG_FIRST_ROUND_MIN_HOLDERS` | 20 | 首轮豁免: 持币数 ≥ 20 |
| `TAG_FIRST_ROUND_MIN_PROGRESS` | 0.15 | 首轮豁免: 进度 ≥ 15%（未毕业） |
| `TAG_FIRST_ROUND_MIN_LIQUIDITY` | 5000 | 首轮豁免: 流动性 ≥ $5k（已毕业） |
...
代币买入时自动检测交易场所，选择对应的买入方式：

| 场所 | 检测方式 | 买入方式 |
|------|----------|----------|
| four.meme bonding curve | `fm_get_token_info()` → offers > 0 | `fm_buy_token()`: 根据 quote 选 BNB/USDT |
| flap bonding curve | `flap_get_token_state()` → status=1/2 | `flap_buy_token()` → Router `0x27772d13` (统一用 BNB) |
| PancakeSwap | liquidityAdded=true 或 status=4 | `buy_token()`: BNB → Token |
