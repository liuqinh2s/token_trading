...
交易策略差异：
- 止盈：回撤止盈（涨30%触发，中点止盈法），与潜伏型一致
- 止损：-20%（与潜伏型一致，数据驱动统一止损线）

## 卖出策略

| # | 策略 | 条件 | 说明 |
|---|------|------|------|
| 1 | 回撤止盈 (中点止盈法) | 涨 30% 触发跟踪, 跌到 (买入价+最高价)/2 卖出 | 让利润奔跑, 回撤到中点落袋 |
| 2 | 动能衰竭止盈 | ≥2 个动能指标恶化 且当前盈利 | 趁还有利润赶紧跑 |
| 3a | 超期清仓 (亏损) | 持仓 ≥ 48h 且仍亏损 | 给了足够时间还亏就别等了 |
| 3b | 超期清仓 (低效) | 持仓 ≥ 72h 且盈利 < 500% (5倍) | 资金效率太低，回收去抓下一个 |
| 4 | 兜底止损 | 亏损 ≥ 20% | 数据驱动: -20%全量回测+1736% vs -60%回测-1121% |
...
| 配置参数 | 默认值 | 说明 |
|----------|--------|------|
| `tp_trigger_pct` | 30 | 回撤止盈触发阈值 (%, 涨幅达到此值后激活中点止盈跟踪) |
...
| `stop_loss_pct` | -20 | 兜底止损 (%, 数据驱动: -20%全量回测+1736%) |
| `grad_stop_loss_pct` | -20 | 毕业通道止损 (%, 与潜伏型统一) |
